...
export function AvatarFallback({ className, ...props }: React.ComponentProps<typeof AvatarPrimitive.Fallback>) {
  return (
    <AvatarPrimitive.Fallback
      className={cn("flex h-full w-full items-center justify-center bg-gradient-to-br from-cyan-300 to-orange-300 text-sm font-semibold text-slate-950", className)}
      {...props}
    />
  );
}
